...
class _HomeScreenState extends State<HomeScreen> {
  ...
  List<String> images = [
    'assets/images/present_location.png',
  ...
  double bearing = Geolocator.bearingBetween(23.845832237942982,
      90.3631272043976, 23.8285983655763, 90.36407714584722);

  double distanceInMeters = Geolocator.distanceBetween(23.83956161087677,
      90.35744835917211, 23.819008976654786, 90.36525413323442);
            ...
            child: Column(
              children: [
                TextField(
                  ...
                  onTap: () {
                    startLocation = stringToLatLng(
                        sourceTEController.text.toString().trim());
                ...
                TextField(
                  ...
                  onTap: () async {
                    ...
                    endLocation = stringToLatLng(
                        destinationTEController.text.toString().trim());
                    // _addRoute(_startLocation, _endLocation);
                  },
                  decoration: const InputDecoration(
                    filled: true,
            ...
            onPressed: () {
              googleMapController.animateCamera(CameraUpdate.newCameraPosition(
                const CameraPosition(
                  zoom: 16,
                  target: LatLng(23.730949422298362, 90.42521660684274),
                ),
              ));
            },
          ...
          FloatingActionButton.extended(
            onPressed: () async {
              ...
              polyLines.add(Polyline(
                  polylineId: const PolylineId('path-to-something'),
                  points: <LatLng>[
                    LatLng(position.latitude, position.longitude),
                    LatLng(
                        position.latitude + 0.004, position.longitude + 0.0007),
                    LatLng(
                        position.latitude + 0.004, position.longitude + 0.005),
                    LatLng(
                        position.latitude - 0.009, position.longitude + 0.0007),
  ...
  void _addRoute(LatLng start, LatLng end) async {
    ...
    if (data['routes'].isNotEmpty) {
      ...
      final decodedPoints =
          DrawRouteSourceToDestination().decodePolyline(route);
